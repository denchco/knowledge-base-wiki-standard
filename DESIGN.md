...
- Canonical entry and architecture diagrams use one top-to-bottom reading direction, Mermaid's native `basis` connectors, consistent rectangular node geometry, ordinary arrows, no edge labels, no more than five nodes and five visible relationships, and at most one parallel branch. Reference diagrams inherit the renderer's node, rank, and padding defaults. Only bounded overrides proven necessary to fit the content rail or preserve legibility may depart from those defaults, and the reason must be recorded. Put finer distinctions in nearby prose or a separate diagram.
