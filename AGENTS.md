...
- The managed Standard preview exact-matches all twelve governed registration fields, the generated installed LaunchAgent, loaded job, canonical URL, and static identity marker. Use `npm run service:preview` for fail-closed browser handoff. Live service status is maintainer-only evidence; distribution/CI records `DKBWS-RUNTIME-002` as not checked.
...
- Treat inline/reference/autolink Markdown, HTML `a`/`area` links, and plain GFM-autolinked HTTP(S) text as response navigation. `file://` URLs, IDE or editor URLs, clickable local-filesystem paths, and repository-relative Markdown links MUST NOT substitute for Wiki navigation. If a route is unavailable, report that state rather than falling back to a file link.
...
- When the response-link checker is present, validate a captured draft with `npm run response:links:check -- --base-url <canonical-url>`. For a managed local Wiki, add `--managed-live` so the exact registered service and HTTP 200 for every displayed route are checked. This does not remove the `STD-VAL-007` host pre-send limitation for responses the host never exposes.
...
- Validate direct lifecycle-state invariants rather than trusting command history, and treat issue linkage, upstream decision, release inclusion, and adoption as terminal workflow-v1 transitions. Record each in a separate JJ commit. Before recording release inclusion, read the registry at the exact full `registryRevision`, prove it strictly descends from the release revision, require the exact public tag with `draft: false` and `immutable: true`, and resolve that tag to the release revision.
- `pre-registry-local-history` is reserved only for the exact immutable UK Digital Health `DKBWS-LINK-002` bridge already in the Standard registry; never use it for a new proposal record or registry entry. A closed issue is not acceptance, acceptance is not a release, and a release never authorises a consumer pin change or adoption.
