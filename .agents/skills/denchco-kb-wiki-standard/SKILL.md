...
When `DKBWS-PROMPT-002` applies, present user-facing Human or LLM Wiki navigation only as live absolute HTTP(S) URLs rooted at the target Wiki's configured canonical URL: the manifest URL when declared, otherwise the registered canonical managed-preview endpoint. Verify each displayed route against the live service during the turn. Never substitute a `file://` URL, IDE or editor URL, clickable local-filesystem path, or repository-relative Markdown link. A plain repository path is permitted only to identify an implementation artifact with no Wiki route and must not be presented as Wiki navigation. When the checker is present, lint a captured draft with `npm run response:links:check -- --base-url <canonical-url>`.
