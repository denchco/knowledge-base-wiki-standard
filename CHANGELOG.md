...
- Simplified the reference Mermaid diagrams into short, predominantly linear top-to-bottom flows with native `basis` connectors, consistent rectangular node geometry, ordinary arrows, `10px` node padding, no edge labels, and no cluster-title crossings. Static complexity limits and rendered text/connector collision checks enforce the clearer default-first layout under `DKBWS-DESIGN-001`.
- Migration: existing consumers are unchanged. New or adapted entry/architecture diagrams should inherit Mermaid's native curve, start from `10px` node padding, keep to five nodes and five relationships, use at most one parallel branch, use consistent rectangular nodes and ordinary arrows, and move detailed distinctions into nearby prose or a separate diagram.
