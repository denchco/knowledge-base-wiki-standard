...
The protected `v0.1.0-rc.4` publication gate is closed by the exact immutable release and this strictly later registry ledger. Absolute interception of uncaptured responses is parked until a selected host exposes a governed pre-send hook, future compatibility work is parked until another exact adapter or environment is deliberately selected, and a dedicated Standard MCP remains a no-go because the observed discovery and update-inspection tasks are served by the local CLI, Git, and governed GitHub issue route.
