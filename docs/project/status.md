...
description: Current implementation and limitations.
status: draft
---
...
| Dependency contract | Exact Node `24.20.0` local/CI enforcement plus reference-qualified Mermaid `11.17.2`, DOMPurify `3.4.15`, Zensical `0.0.59`, Graphify `0.9.55`, retained vis-network `10.1.2`, installation classes, offline boundary, consumer/maintainer separation, and reserved verification-artifact rules implemented |
...
| Development-response navigation | Exact `development_response_links: live-wiki` capability, canonical Human Wiki origin, managed root/starter instructions, Markdown/entity-decoded-HTML/plain-GFM detection, and `--managed-live` exact identity plus per-route HTTP checks implemented; the optional trusted Codex Stop adapter supports one corrective continuation; absolute interception remains unclaimed under `STD-VAL-007` |
...
| Standard-change intake | `DKBWS-UPDATE-002` released in `v0.1.0-rc.4` with tracked versioned records, direct-state and terminal-transition invariants, deterministic payload/form approval, exact issue linkage, exact registry-revision bytes and strict release ancestry, exact public non-draft immutable release checks, one fixed historical bridge, and separately authorised adoption; this later ledger maps the historical bridge without fabricating an issue or implying consumer adoption, remote reconciliation is GET-only, no command performs a remote write, and the user performs final submission |
...
| Public GitHub release | Public `denchco` repository with a no-bypass pull-request/merge-commit `main` ruleset, required exact GitHub Actions `verify` check, deletion/force-push protection, full-SHA selected Actions, read-only workflow permissions, secret scanning and push protection, private vulnerability reporting, and native immutable releases; `v0.1.0-rc.4` remains the preserved previous immutable prerelease at exact attested revision `4aae4d4efbcea526f28ec8bcbfaf203dae34bc25` and includes the tracked proposal, live-response, managed-preview, governing-question, reader-navigation, absolute JJ-turn, and exact supported-toolchain contracts |
...
This revision prepares `v0.1.0-rc.5` with the corrected runtime bundle, canonical OKF source identity, exact reference environment, optional Codex adapter and path-scoped discovery. Release authority is the immutable public tag at its exact verified commit. The previous `rc.4` remains unchanged.

No recurring priorities are created by this release. The publication workflow requires exact-reference local and protected CI qualification, an immutable Standard release, and Documentation synchronization to that exact release commit before publication. The GitHub release record establishes completion of that workflow; this source does not invent a future commit hash.

Absolute interception of uncaptured responses remains outside the optional Stop continuation adapter. A dedicated Standard MCP remains deferred because the demonstrated workflows are served by repository files, CLI, skill/plugin and optional read-only integrations.
