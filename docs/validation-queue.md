...
| STD-VAL-007 | The repository can reject a captured development-response draft that violates `DKBWS-PROMPT-002`, including HTML/plain-GFM destinations and failed managed identity/routes, but it cannot absolutely intercept an uncaptured model response before delivery. | Parked | None. Keep captured-draft checks, `--managed-live`, managed-instruction parity, and violation-as-nonconformance in force. | The selected agent host provides no governed repository-level pre-send hook or mandatory response artifact. | A selected host exposes a pre-send validation hook or mandatory response artifact that can invoke the checker. | 2026-08-09 |
