...
- Extend the OKF v0.2 validator only when upstream format changes are evidenced; the current hard boundary, guidance diagnostics, lossless export, and fixtures are executable.
- Broaden the built-output browser matrix beyond the current desktop/mobile, diagram, graph, local-network, geometry, and console contracts.
