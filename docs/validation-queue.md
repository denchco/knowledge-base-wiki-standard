---
type: Validation Queue
title: Validation queue
description: Outstanding evidence and implementation work.
status: draft
---

# Validation queue

| ID | Gap or decision | State | Next bounded action | `blocked_by` | `reopen_when` | `last_checked` |
|---|---|---|---|---|---|---|
| STD-VAL-001 | Future OKF v0.2 changes may require validator changes. | Parked | None. Retain the current hard boundary, guidance diagnostics, lossless export, and fixtures. | No evidenced upstream format change. | An upstream OKF revision or erratum changes a supported rule. | 2026-08-03 |
| STD-VAL-002 | Built-output coverage may need environments beyond the current desktop/mobile, diagram, graph, local-network, geometry, table-overflow, and console contracts. | Parked | None. The second consumer supplied route, graph fail-closed, draft row-centre, and mobile-table gaps, all now covered. | No current failing environment. | A consumer or supported adapter supplies another reproducible gap. | 2026-08-04 |
| STD-VAL-003 | Zensical and graph-publication compatibility needs versioned evidence. | Parked | None. The second consumer used the already supported exact adapter pair. | No new exact version pair is selected. | A consumer or intentional dependency upgrade selects a new pair. | 2026-08-04 |
| STD-VAL-004 | Jujutsu `0.42.0` is not reference-qualified. | Parked | Retain `0.39.0`. | Provenance and complete Standard Production verification have not been recorded for `0.42.0`. | An intentional Jujutsu qualification pass is in scope. | 2026-08-03 |
| STD-VAL-005 | Five dogfood audits are inferred rather than project-owned conformance claims. | Parked | None at Standard level. | Each project owner has not made an explicit adoption decision. | A project owner explicitly requests formal adoption. | 2026-08-03 |
| STD-VAL-006 | The local `DKBWS-UPDATE-002` proposal workflow, governed issue form, expected `standard-change` label, and registry bridge are not yet included in an immutable public Standard release. | Parked | None. Retain fail-closed public form/label checks and the current consumer pins. | Publication, label creation, release, and adoption each require separate authority. | The user explicitly authorises the relevant publication or immutable release phase. | 2026-08-07 |
| STD-VAL-007 | The repository can reject a captured development-response draft that violates `DKBWS-PROMPT-002`, but it cannot absolutely intercept an uncaptured model response before delivery. | Parked | None. Keep captured-draft checks, managed-instruction parity, and violation-as-nonconformance in force. | The selected agent host provides no governed repository-level pre-send hook or mandatory response artifact. | A selected host exposes a pre-send validation hook or mandatory response artifact that can invoke the checker. | 2026-08-07 |

Parked rows are not recurring handoff recommendations. Reopen one only when its recorded trigger occurs or the user explicitly changes scope.
