...
- **Human and Agent** (`human-and-agent`) — coordinated Human Wiki and operational LLM Wiki; any displayed governing-question callout is semantically marked and uses the active accent for both its rail and text, every exact reader-facing repetition of a declared canonical question is governed across its bounded reader set, ordinary quotations and paraphrases remain neutral, any displayed registered source identity navigates to its exact internal source-register row, and development responses use verified live Wiki URLs instead of file links for Wiki navigation.
- **Standard Production** (`standard-production`) — Zensical, DenchCo style, local Mermaid, Graphify 2D/3D, browser verification, an identity-verified managed local service, tracked Standard-change intake, Git interoperability, and a colocated Jujutsu commit at the end of every file-changing development turn.
...
`DKBWS-PROMPT-002` gives the Human and Agent profile an exact `development_response_links: live-wiki` capability. A matching absolute `human_wiki_url`, managed instruction parity, response-link lint across Markdown, entity-decoded HTML links, and plain GFM autolinks, plus `--managed-live` exact service identity and per-route HTTP evidence make the preference testable. A host without a pre-send hook can still emit a nonconforming response; the Standard detects captured violations and makes the obligation explicit, but it cannot intercept conversational output the host never exposes. `STD-VAL-007` remains parked until a selected host exposes that boundary.
...
`DKBWS-RUNTIME-002` makes the selected Standard Production `local_service` capability executable rather than descriptive. The platform adapter may vary, but serialized and atomically published shared-register ownership, exact comparison of every declared registration field, exact installed/loaded user-service job, versioned marker identity, and fail-closed canonical `service:preview` handoff do not. Live current-service evidence belongs only to maintainer verification; distribution/CI reports the requirement and capability as `not-checked`. A project choosing only a foreground runtime must select a profile or deviation where the persistent managed requirement does not apply.

`DKBWS-UPDATE-002` makes the selected Standard Production `standard_change_intake` capability durable rather than conversational. The tracked proposal record is authoritative; generated payloads remain disposable. Directly loaded records must satisfy the same state metadata, prerequisite, ordering, and terminal-transition invariants as command-produced records. Explicit decision, immutable-release inclusion, and later consumer adoption are independent states, and no GitHub issue closure can substitute for acceptance. Release evidence is exact: validated registry bytes at the full recorded revision, strict release-to-registry ancestry, a matching non-draft immutable public release, and an exact tag-to-revision resolution. The sole historical bridge is the immutable workflow-v1 UK Digital Health `DKBWS-LINK-002` entry; new work cannot select that intake mode. Validation and complete verification prove these controls, disclosure safety, and absence of remote writes.
