...
`DKBWS-UPDATE-002` makes the selected Standard Production `standard_change_intake` capability durable rather than conversational. The tracked proposal record is authoritative; generated payloads remain disposable. Explicit decision, immutable-release inclusion, and later consumer adoption are independent states, and no GitHub issue closure can substitute for acceptance. Validation and complete verification prove deterministic payload binding, transition safety, disclosure controls, and the absence of remote writes.
