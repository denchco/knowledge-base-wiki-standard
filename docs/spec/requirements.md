...
| DKBWS-PROMPT-001 | Clarification MUST be sequential, individually numbered, and evidence-dependent. The bootstrap prompt MUST accept the standard repository without requiring a user-supplied revision; the implementation MUST resolve its current default-branch HEAD to an exact commit SHA and pin that SHA before acting. Agent handoffs MUST be completion-first, offer no more than three goal-linked and executable recommendations, apply a parked-work cycle guard, and never invent adjacent work to fill a list. | Standard prompts, initialization planning, and agent profiles | Instruction parity, automatic-revision initializer test, and prompt fixture |
