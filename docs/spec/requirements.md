...
| DKBWS-RUNTIME-002 | A persistent managed Wiki preview MUST own one stable service identity and canonical loopback endpoint in a shared local register. The load, conflict checks, live-listener probe, and publication of a reservation MUST be one serialized transaction under an exclusive user-local lock, and the complete register MUST be replaced atomically. Registration MUST reject both registered endpoint conflicts and unmanaged live listeners without stopping or reassigning them. Identity health MUST require HTTP 200 from a versioned static marker whose service ID exactly matches the registered service. Managed status MUST fail unless registry ownership, adapter installation, adapter load, and identity health all match. A requested browser preview MUST use that exact registered canonical URL; an ephemeral alternate-port check MUST NOT prove this requirement. | Persistent managed runtime adapters | Marker schema, lock/atomic-write/registry/listener/status fixtures, adapter state inspection, and exact-URL browser handoff |
...
The shared register is a user-scoped, adapter-documented inventory with a versioned root and entries keyed by stable service ID. Each entry binds that identity to the owning project root, loopback host, port, canonical URL, command, adapter label, and marker health path. The Standard's macOS reference adapter stores register version `1` under the user's configuration directory and uses a user LaunchAgent; another operating-system adapter MAY choose its native user-service mechanism and register location while preserving the same ownership and deconfliction semantics.
...
5. Install and load the selected user-service adapter. Status passes only when the exact registration, installed adapter, loaded adapter, and identity marker response all agree.
6. When the user asks for a preview, open or hand off the registered canonical URL in the selected browser and verify the page there. An available port chosen only for a test process is separate evidence for `DKBWS-RUNTIME-001`.
