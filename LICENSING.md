# Licensing

Copyright (c) 2026 Andrew Dench.

Unless a file states otherwise, the repository's original code, schemas, scripts, workflows, fixtures, adapters, templates, specification, and accompanying documentation are licensed under the MIT License in `LICENSE`.

The MIT License permits private, public, modified, and commercial use while requiring the copyright and permission notice to remain with copies or substantial portions. It supplies the work without warranty.

Third-party dependencies, generated vendor bundles, quoted material, examples derived from upstream projects, and registered sources retain their own licences or usage terms. This repository does not relicense them. Do not redistribute source material merely because it is cited or was available to an agent.

Tracked consumer-derived Standard proposal records identify relevant third-party licence evidence and link to redistributable public material rather than copying restricted content. Before a payload is approved, proposal checks reject private paths and material the originating project lacks authority to share. Generated payloads and the governed form never upload patches, screenshots, or evidence automatically. A proposal, issue, acceptance, or release does not transfer ownership or expand reuse rights.

The MIT licence does not transfer ownership of the DenchCo name or presentation identity. The `docs/assets/brand/denchco-wordmark.png` asset is included to identify this Standard and its Documentation; it is not part of the subject-empty consumer starter. Ordinary identification of the source is allowed; broader branding permissions are not part of this software licence.

## Bundled third-party assets

| Asset | Source | Licence | Use |
|---|---|---|---|
| `docs/assets/pen-circle.svg` | [Pen Circle by SVG Repo](https://www.svgrepo.com/svg/532983/pen-circle) | [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) | Draft-status marker in the Zensical reference navigation adapter |
