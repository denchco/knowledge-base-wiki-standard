...
No rendered page may load Mermaid, vis-network, or 3d-force-graph from a public CDN. The browser contract resolves question, Mermaid, architecture, table, list, and graph representatives independently; records requests; fails closed when a selected-profile graph route is absent; and rejects public-CDN use, failed local assets, console errors, blank diagrams/canvases, unusable controls, uncontained mobile tables, or desktop/mobile overflow.

Adapters whose homepage does not contain every representative shape set local-path-only `DKBWS_BROWSER_QUESTION_ROUTE`, `DKBWS_BROWSER_MERMAID_ROUTE`, `DKBWS_BROWSER_ARCHITECTURE_ROUTE`, `DKBWS_BROWSER_TABLE_ROUTE`, and `DKBWS_BROWSER_LIST_ROUTE` values. Graph routes use the corresponding `_GRAPH_2D_ROUTE` and `_GRAPH_3D_ROUTE` overrides. Mermaid source inspection is inferred from the route or supplied through `_MERMAID_SOURCE` and `_ARCHITECTURE_SOURCE`; remote, query-bearing, fragment-bearing, and traversal values are rejected.
