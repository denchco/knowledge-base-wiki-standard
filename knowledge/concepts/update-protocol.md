...
Human-and-Agent consumers expose development-response Wiki navigation only through their configured live canonical HTTP(S) origin. Their agent instructions and response-link checker reject file, editor, absolute-path, and repository-relative Markdown links as substitutes for rendered Wiki routes. Managed local routes are identity- and HTTP-verified during the turn; an unavailable route is disclosed rather than silently replaced with a source-file link.
